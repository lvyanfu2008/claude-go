...
- **`conversation-runtime/query/`** — Port of `src/conversation-runtime/query.ts`: compaction, `Query`, optional **`LocalTurnCallModel`** (ccb-engine), and optional **streaming parity** (`runStreamingParityModelLoop`: Anthropic SSE + `streamingtool` + `toolexecution`). Streaming uses **`goc/anthropicmessages.PostStream`**, which honors **`CLAUDE_CODE_LOG_API_REQUEST_BODY`** / **`CLAUDE_CODE_LOG_API_RESPONSE_BODY`** via **`ccb-engine/apilog`** (request JSON + raw SSE body, capped for response size).
