...
func (m *Model) renderMessagePaneWithNewRenderer() string {
	...
	if m.uiScreen != gouDemoScreenTranscript {
		...
		if len(tailLines) > 0 {
			if content != "" {
				content += "\n"
				// Only add blank line before streaming text, not before ⎿ (tool use)
				if len(messagesPtr) > 0 && messagesPtr[len(messagesPtr)-1].Type != types.MessageTypeAssistant && !strings.HasPrefix(tailLines[0], "  ⎿") {
					content += "\n"
				}
			}
