...
func IsAutoMemoryEnabled() bool {
	...
	if claudebase.EnvDefinedFalsy(envVal) {
		return true
	}
	if claudebase.Truthy(os.Getenv("CLAUDE_CODE_SIMPLE")) {
		return false
	}
	if claudebase.Truthy(os.Getenv("CLAUDE_CODE_REMOTE")) && strings.TrimSpace(os.Getenv("CLAUDE_CODE_REMOTE_MEMORY_DIR")) == "" {
		return false
	}
...
func autoMemBaseKey(absCwd string) string {
	if cr := claudebase.ResolveCanonicalGitRoot(absCwd); cr != "" {
		return claudebase.SanitizePath(cr)
	}
	return claudebase.SanitizePath(absCwd)
}
