...
Format: `timestamp [API_REQUEST_BODY|API_RESPONSE_BODY] <label>\nllmRequest-N<compact JSON>\n----------\n` (response: `llmResponse-M…`). Tag is concatenated to the body line with no space so grep matches one line.

**Output file** (first match, same order as TS `getDebugLogPath` / [`debugpath.ResolveLogPath`](debugpath/path.go)): `CLAUDE_CODE_DEBUG_LOG_FILE` if set; else `CLAUDE_CODE_DEBUG_LOGS_DIR` (directory → `<dir>/<sessionId>.txt`, same heuristics as TS); else `$CLAUDE_CONFIG_DIR/debug/<sessionId>.txt` or `~/.claude/debug/<sessionId>.txt`. You do **not** need a fixed filename: **`latest`** in that same directory is a symlink updated on each append to point at the active log file (open `~/.claude/debug/latest` when using defaults). **`ccb-engine` and `gou-demo` call `apilog.PrepareIfEnabled()` after loading project settings**, which creates the debug directory (and an empty log file) as soon as either `CLAUDE_CODE_LOG_API_*` flag is on. The first prepare or append prints `[ccb-engine apilog] LLM API bodies: …/latest points to …` on stderr. Set overrides in project `.claude/settings.go.json` `env` (merged on startup for Go; see above). To share one file with Bun, set `CLAUDE_CODE_DEBUG_LOG_FILE` to the same path TS uses.
