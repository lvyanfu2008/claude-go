...
func TestRunToolUseChan_bashWholeToolAsk1bSkipsAskWhenSandboxFlags(t *testing.T) {
	perm := types.EmptyToolPermissionContextData()
	...
	raw := json.RawMessage(`[{"name":"Bash","input_schema":{"type":"object","properties":{"command":{"type":"string"}},"required":["command"]}}]`)
	...
	ch := RunToolUseChan(context.Background(),
		streamingtool.ToolUseBlock{ID: "t1", Name: "Bash", Input: json.RawMessage(`{"command":"echo hi"}`)},
		types.Message{Type: types.MessageTypeAssistant, UUID: "asst"},
		deps,
		nil,
	)
	for u := range ch {
		if u.Message != nil && u.Message.Type == types.MessageTypeUser {
			...
			if strings.Contains(body, "Permission required") {
				t.Fatalf("unexpected whole-tool ask block: %s", body)
			}
		}
	}
	if !invoked {
		t.Fatal("expected InvokeTool after 1b bypass")
	}
}
