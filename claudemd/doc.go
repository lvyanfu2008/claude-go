...
//
// Settings: project .harness/settings.json is TS-only. Go uses ~/.harness/settings.go.json (user),
// <cwd>/.harness/settings.go.json, and settings.local.json for env (settingsfile) and claudeMdExcludes (MergedClaudeMdExcludes).
...
//   - User ~/.harness/settings.go.json or cowork_settings.json (CLAUDE_CODE_USE_COWORK_PLUGINS=1).
