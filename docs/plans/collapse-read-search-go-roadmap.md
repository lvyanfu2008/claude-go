...
- **`CollapseReadSearchGroupsInList`**: implemented in `gou/messagerow` (Ink parity) but **not** invoked from `MessagesForScrollList` (main list keeps separate Read/Grep/Glob rows). Do not combine with tail merge on the same store unless you understand double-collapse risk.
