...
| `collapsed_read_search` rollup | [`messagerow/collapse_roll_up.go`](../../gou/messagerow/collapse_roll_up.go), [`ccbstream/apply.go`](../../gou/ccbstream/apply.go) | **Opt-in:** `GOU_DEMO_COLLAPSE_READ_SEARCH_TAIL=1` runs after each **`tool_result`**: the tail of **Read / Grep / Glob** `tool_use` + matching `tool_result` pairs is replaced by one **`collapsed_read_search`** row (TS `collapseReadSearchGroups` subset). Default off so gou-demo keeps per-tool rows in the list. Not in MVP: Bash list/git, memory/team splits, MCP, Snip/ToolSearch absorption. |
