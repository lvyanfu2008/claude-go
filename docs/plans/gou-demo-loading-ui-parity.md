...
| Builtin status (model · Context · $ · Debug) | [`cmd/gou-demo/builtin_status_line.go`](../../cmd/gou-demo/builtin_status_line.go) | Mirrors TS `BuiltinStatusLine` shape: model from last submit / env; **Context %** from rough token estimate (`len(content)/4` sum vs `GOU_DEMO_CONTEXT_WINDOW` default 200k); **`GOU_DEMO_SESSION_COST_USD`** for cost segment; **Debug mode** when `GOU_DEMO_DEBUG` or `CLAUDE_CODE_DEBUG`. Disable with **`GOU_DEMO_NO_BUILTIN_STATUS=1`**. |
