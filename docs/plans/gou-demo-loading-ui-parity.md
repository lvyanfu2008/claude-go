...
| `collapsed_read_search` rollup | [`messagerow/collapse_roll_up.go`](../../gou/messagerow/collapse_roll_up.go), [`messagerow/collapse_read_search_groups.go`](../../gou/messagerow/collapse_read_search_groups.go), [`messagerow/bash_search_read.go`](../../gou/messagerow/bash_search_read.go), [`messagerow/search_read_summary.go`](../../gou/messagerow/search_read_summary.go), [`ccbstream/apply.go`](../../gou/ccbstream/apply.go) | **Tail:** `GOU_DEMO_COLLAPSE_READ_SEARCH_TAIL=1` on **`tool_result`**. **Main scroll list** does not merge consecutive Read/Grep/Glob via `CollapseReadSearchGroupsInList`. **Bash:** `isSearchOrReadBashCommand` or **`GOU_DEMO_COLLAPSE_ALL_BASH=1`**. Transcript: **`VerboseCollapsedReadSearch`** nested rows. Still not in Go: TS auto fullscreen, full TS hook/memory/MCP. Roadmap: [collapse-read-search-go-roadmap.md](./collapse-read-search-go-roadmap.md). |
...
- `go test ./gou/messagesview/...` — list pipeline (no full-list read/search rollup in scroll path).
