...
| `collapsed_read_search` rollup | [`messagerow/collapse_roll_up.go`](../../gou/messagerow/collapse_roll_up.go), [`messagerow/bash_search_read.go`](../../gou/messagerow/bash_search_read.go), [`messagerow/search_read_summary.go`](../../gou/messagerow/search_read_summary.go), [`ccbstream/apply.go`](../../gou/ccbstream/apply.go) | **Opt-in:** `GOU_DEMO_COLLAPSE_READ_SEARCH_TAIL=1` runs after each **`tool_result`**: tail merge into **`collapsed_read_search`**. **Bash** included when **`isSearchOrReadBashCommand`** (search/read/list), or any Bash when **`GOU_DEMO_COLLAPSE_ALL_BASH=1`** (`CollapseAllBashFromEnv`, TS fullscreen-style **`bashCount`**). Summary line includes **Ran/ran N bash command(s)** via `SearchReadSummaryText`. Still not in Go: TS auto fullscreen env, memory/team/MCP, Snip/ToolSearch, hooks. Roadmap: [collapse-read-search-go-roadmap.md](./collapse-read-search-go-roadmap.md). |
