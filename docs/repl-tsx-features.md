...
| `AlternateScreen`、`FullscreenLayout` | 交替屏、高度约束、与转写虚拟滚动分支共用。Go 默认主缓冲；`GOU_DEMO_ALT_SCREEN=1` 时 `tea.WithAltScreen`（备屏），滚轮更易作用在消息区；复制仍可用 Shift+左拖选区 + Ctrl+C（应用内） |
| `AnimatedTerminalTitle`、`useTerminalTitle` | 动态终端标题；可 `CLAUDE_CODE_DISABLE_TERMINAL_TITLE` |
| 虚拟滚动 `VirtualMessageList` + `scrollRef` + `useVirtualScroll` | TS：`VirtualMessageList` 内用 `useVirtualScroll` 算可见区间；`scrollRef` 为 Ink `ScrollBoxHandle`（`scrollTo` / `scrollToBottom` / `getScrollTop` 等）。Go：新消息渲染器 [`goc/gou/message.VirtualList`](../gou/message/) 统一驱动主会话屏与转写屏，[`cmd/gou-demo/main.go`](../cmd/gou-demo/main.go) 的 `scrollTop` / `sticky` / `pendingDelta` + `View` 对应「列表 + 命令式滚动」的合并效果（无独立 ref 对象）。**鼠标**：`tea.WithMouseCellMotion`，消息区滚轮与左键拖拽改 `scrollTop`（[`mouse_message_list.go`](../cmd/gou-demo/mouse_message_list.go)）；`GOU_DEMO_DISABLE_MOUSE_SCROLL=1` 关闭。主会话屏默认使用 [`bubbles/viewport`](https://github.com/charmbracelet/bubbles) 承载新渲染器输出的整段文档滚动（与示例 [`go-tui`](../go-tui/main.go) 同源思路）；`ctrl+y` 折叠摘要行；`GOU_DEMO_BUBBLES_VIEWPORT=0` 时直接按 `m.scrollTop` 切片渲染可见区间 |
