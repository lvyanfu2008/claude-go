package hookexec

import (
	"context"
	"encoding/json"

	"goc/types"
)

const hookEventConfigChange = "ConfigChange"

type configChangeHookInput struct {
	BaseHookInput
	Source     string `json:"source"`
	ConfigFile string `json:"config_file"`
}

// RunConfigChangeHooks executes ConfigChange command hooks.
// Mirrors TS executeConfigChangeHooks in src/utils/hooks.ts.
func RunConfigChangeHooks(
	ctx context.Context,
	table HooksTable,
	workDir string,
	base BaseHookInput,
	source, configFile string,
	batchTimeoutMs int,
) ([]types.AggregatedHookResult, error) {
	if HooksDisabled() || ShouldDisableAllHooksIncludingManaged() || ShouldSkipHookDueToTrust() {
		return nil, nil
	}

	in := configChangeHookInput{
		BaseHookInput: base,
		Source:        source,
		ConfigFile:    configFile,
	}
	in.HookEventName = hookEventConfigChange

	jsonIn, err := marshalHookInput(in)
	if err != nil {
		return nil, err
	}

	var hookInput map[string]any
	if err := json.Unmarshal([]byte(jsonIn), &hookInput); err != nil {
		return nil, err
	}
	if len(CommandHooksForHookInput(table, hookInput)) == 0 {
		return nil, nil
	}

	wd := trimOrDot(workDir)
	results := ExecuteCommandHooksOutsideREPLParallel(OutsideReplCommandParams{
		Ctx:       ctx,
		WorkDir:   wd,
		Hooks:     table,
		JSONInput: jsonIn,
		TimeoutMs: batchTimeoutMs,
	})

	var agg []types.AggregatedHookResult
	for _, r := range results {
		agg = append(agg, hookAggregate(r, randomUUID(), hookEventConfigChange, r.Command)...)
	}
	return agg, nil
}
