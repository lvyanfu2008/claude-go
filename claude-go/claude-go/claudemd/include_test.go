...
func TestExtractIncludePaths(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			paths := ExtractIncludePathsFromMarkdown([]byte(tt.content), tt.baseDir)
...
func TestIncludeInMemoryFiles(t *testing.T) {
	...
	if strings.Contains(out, "ignored.md") {
		t.Error("Code block include should not be processed")
	}
