...
const askUserQuestionBasePrompt = `Use this tool when you need to ask the user questions during execution. This allows you to:
...
const askUserQuestionPreviewMarkdown = `
Preview feature:
...
const askUserQuestionPreviewHtml = `
Preview feature:
Use the optional ` + "`preview`" + ` field on options when presenting concrete artifacts that users need to visually compare:
...
// getAskUserQuestionDescription mirrors AskUserQuestionTool.prompt() in TS.
// It returns the base prompt, and appends the preview feature prompt based on
// the current QuestionPreviewFormat (set via SetQuestionPreviewFormat).
func getAskUserQuestionDescription() string {
	switch GetQuestionPreviewFormat() {
	case "html":
		return askUserQuestionBasePrompt + askUserQuestionPreviewHtml
	case "markdown":
		return askUserQuestionBasePrompt + askUserQuestionPreviewMarkdown
	default:
		// Undefined / not opted in — omit preview guidance (SDK consumers
		// may not render the preview field at all).
		return askUserQuestionBasePrompt
	}
}
...
func nativeListPeersToolSpec() types.ToolSpec {
	...
	builder.WriteString("List active Harness Code sessions that can receive messages via SendMessage.\n\n")
