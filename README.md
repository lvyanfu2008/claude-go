...
- The built-in **`Bash`** row from embed **`tools_api.json`** is **removed** from the tool list and replaced by **`BashZog`** (same local shell execution in the Go runner). **`description` / `input_schema`** for that row are maintained in Go ([`ccb-engine/bashzog/wire.go`](ccb-engine/bashzog/wire.go) plus [`ccb-engine/bashzog/tool.go`](ccb-engine/bashzog/tool.go)), same style as native tool specs in [`toolpool/go_tool_wire.go`](toolpool/go_tool_wire.go); **Zog** validates **`BashZog`** in [`zoglayer`](internal/zoglayer) plus [`toolrefine`](internal/toolrefine), aligned with the TS `fullInputSchema` **wire** (including optional `_simulatedSedEdit`).
