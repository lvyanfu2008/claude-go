...
- **[`toolpool.ReplaceBashToolSpecIfZogMode`](toolpool/bash_zog.go)** (used by [`AssembleToolPoolFromEmbedded`](toolpool/assemble_embedded.go)) swaps **`Bash` → `BashZog`** on the assembled pool. **[`ToolSpecsFromGoWire`](toolpool/go_tool_wire.go)** / **`GetTools`** then expose **`BashZog`** instead of **`Bash`** in the same way as the main runtime list.
