...
- The built-in **`Bash`** row from embed **`tools_api.json`** is **removed** from the tool list and replaced by **`BashZog`** (same local shell execution in the Go runner). **`description` / `input_schema`** for that row are maintained in Go ([`ccb-engine/bashzog/model_wire_native.go`](ccb-engine/bashzog/model_wire_native.go), loaded by [`ccb-engine/bashzog/tool.go`](ccb-engine/bashzog/tool.go)); **Zog** validates **`BashZog`** in [`zoglayer`](internal/zoglayer) plus [`toolrefine`](internal/toolrefine), aligned with the TS `fullInputSchema` **wire** (including optional `_simulatedSedEdit`).
