...
1. **After tasks.md approval**, execute the appropriate command:
   ...
   **Option 1 - Use the launcher (recommended):**
   ```bash
   ./.harness/scripts/generate-commands-launcher.sh {spec-name}
   ```
   ...
   **Windows:**
   ```cmd
   .harness\scripts\generate-commands.bat {spec-name}
   ```

   **macOS/Linux:**
   ```bash
   ./.harness/scripts/generate-commands.sh {spec-name}
   ```
...
3. **Functionality**: All scripts provide the same functionality:
   ...
   - Generate command files in .harness/commands/{spec-name}/ directory
   - Create individual task commands like /{spec-name}-task-{id}

4. **Integration**: These scripts replace the old `node .harness/scripts/generate-commands.js` command.

### Migration Notes:
- **DO NOT** use `node .harness/scripts/generate-commands.js` anymore
- **DO NOT** reference the JavaScript version in instructions
- **ALWAYS** use the platform-specific scripts or the launcher
- The scripts are generated during setup and stored in `.harness/scripts/`

### Error Handling:
If the launcher fails to detect the OS or find the appropriate script:
1. Check that all three scripts exist in `.harness/scripts/`
