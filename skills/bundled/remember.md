...
Read HARNESS.md and HARNESS.local.md from the project root (if they exist). Your auto-memory content is already in your system prompt — review it there. Note which team memory sections exist, if any.
...
- **Duplicates**: Auto-memory entries already captured in HARNESS.md or HARNESS.local.md → propose removing from auto-memory
- **Outdated**: HARNESS.md or HARNESS.local.md entries contradicted by newer auto-memory entries → propose updating the older layer
...
If auto-memory is empty, say so and offer to review HARNESS.md for cleanup.
