...
// partialSettingsJSON is the minimal shape we need from settings.json (TS SettingsJson.enabledPlugins).
type partialSettingsJSON struct {
...
func MergeEnabledPlugins(cwd string) (map[string]bool, error) {
	...
	projRoot, err := settingsfile.FindClaudeProjectRootAny(cwd)
	if err != nil {
		return nil, err
	}
...
func mergeEnabledPluginsFile(path string, merged map[string]bool) error {
	...
	var p partialSettingsJSON
	if err := json.Unmarshal(raw, &p); err != nil {
		return nil // ignore malformed file like TS may surface elsewhere
	}
