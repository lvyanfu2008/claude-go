...
func resolveSchedule(args string, opt *BundledResolveOptions) (types.SlashResolveResult, error) {
	...
	if org == "" {
		return types.SlashResolveResult{
			UserText: "Unable to resolve organization UUID for the Environment API. Set CLAUDE_CODE_ORGANIZATION_UUID, or ensure ~/.harness/config.json contains oauthAccount.organizationUuid (same sources as Harness Code TS).",
			Source:   types.SlashResolveBundledEmbed,
		}, nil
	}
